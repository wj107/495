...
words, for the \texttt{gamelog} data, for a categorical response variable, the predictive models become more consistent as more data is added to the
test and train sets.  

Taken together, there's a lot of conflicting information:  NN is more accurate for the quantitative response, but GLM is more
...
more consistency (if not accuracy).  Do these trends hold in all cases?  Sounds like topics for future research and analysis!  Find yourself some
data, use the \texttt{se.estimate} function, and dig in!
