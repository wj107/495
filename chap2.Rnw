...
%------simple flowchart???
		...
		\begin{figure}[h!]
			\centering
			\includegraphics[width=4.4in]{{glmchart.png}} 
...
a logistic function.  For responses variables with a Poisson distribution, a log function is used as a link function. 

%%%%---------[read annotte for other links].  figures for link functions???
...
%-------------NN graph!!
		...
		\begin{figure}[h!]
			\centering
			\includegraphics[width=4.4in]{{nnmodel.png}} 
...
%-----------NN graph
		...
		\begin{figure}[h!]
			\centering
			\includegraphics[width=4.4in]{{nnchart.png}} 
...
I am saying that I won't be the one to teach you anything about them.
...
parameters $\beta_i$: minimizing the standard error for the model.  For training a neural net there is a `blind' rule defined for adjusting the 
parameters.  Remarkably, in the end, after repeated iterations, this `blind' rule accomplishes can achieve accurate output.  But is it more 
accurate than GLM?  Fast forward to Section \#3 for details of a head-to-head comparison between the methods\dots
